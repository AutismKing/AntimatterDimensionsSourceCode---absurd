...
export default {
  ...
  computed: {
    ...
    classObject() {
      return {
        "c-break_eternity-upgrade-btn--useless": this.isUseless,
        "c-break_eternity-upgrade-btn--bought": this.isBought && !this.isUseless,
        "c-break_eternity-upgrade-btn--unavailable": !this.isBought && !this.canBeBought && this.isAvailableForPurchase,
        "c-break_eternity-upgrade-btn--possible": !this.isAvailableForPurchase && this.isPossible,
        "c-break_eternity-upgrade-btn--locked": !this.isAvailableForPurchase && !this.isPossible,
      };
    },
    requirementConfig() {
      return {
...
<template>
  <div class="l-spoon-btn-group">
    <button
      :class="classObject"
      class="l-break_eternity-upgrade-btn c-break_eternity-upgrade-btn"
      @click.shift.exact="toggleLock(upgrade)"
      @click.exact="upgrade.purchase()"
    >
      <HintText
        type="BreakEternityUpgrades"
        class="l-hint-text--break_eternity-upgrade c-hint-text--break_eternity-upgrade"
      >
        {{ config.name }}
      </HintText>
      <span :class="{ 'o-pelle-disabled': isUseless }">
        ...
        <template v-if="($viewModel.shiftDown === isAvailableForPurchase) && !isRebuyable">
          ...
          <DescriptionDisplay
            ...
            label="Requirement:"
            class="c-break_eternity-upgrade-btn__requirement"
          />
        </template>
        <template v-else>
          <EffectDisplay
    ...
    <PrimaryToggleButton
      ...
      label="Auto:"
      class="l--spoon-btn-group__little-spoon-break_eternity-btn o-primary-btn--break_eternity-upgrade-toggle"
    />
  </div>
</template>
